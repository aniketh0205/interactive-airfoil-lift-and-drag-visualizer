...
def parse_airfoil_text(text, name="Custom Airfoil"):
    ...
    return {
        ...
        "zero_lift_angle": -abs(max_camber) * 100,
...
def generate_curve_data(airfoil, velocity, air_density, chord_length, wing_span):
    ...
    return data
def generate_csv_data(airfoil, selected_name, angle_of_attack, velocity, air_density, chord_length, wing_span, result):
    """Generate CSV string from simulation data for export."""
    import io
    import csv
    output = io.StringIO()
    ...
    writer.writerow(["Airfoil Analysis Report"])
    writer.writerow([])
    writer.writerow(["Airfoil", selected_name])
    writer.writerow(["Type", airfoil.get("type", "N/A")])
    writer.writerow(["Max Camber", f"{airfoil.get('max_camber', 0)*100:.2f}%"])
    writer.writerow(["Max Thickness", f"{airfoil.get('max_thickness', 0)*100:.1f}%"])
    writer.writerow(["Stall Angle", f"{airfoil.get('stall_angle', 15)}°"])
    writer.writerow(["Zero-Lift Angle", f"{airfoil.get('zero_lift_angle', 0)}°"])
    writer.writerow([])
    writer.writerow(["Input Parameters"])
    writer.writerow(["Angle of Attack", f"{angle_of_attack}°"])
    writer.writerow(["Velocity", f"{velocity} m/s"])
    writer.writerow(["Air Density", f"{air_density} kg/m³"])
    writer.writerow(["Chord Length", f"{chord_length} m"])
    writer.writerow(["Wing Span", f"{wing_span} m"])
    writer.writerow(["Wing Area", f"{S:.2f} m²"])
    writer.writerow([])
    writer.writerow(["Results"])
    writer.writerow(["Lift Coefficient (CL)", f"{result['CL']:.4f}"])
    writer.writerow(["Drag Coefficient (CD)", f"{result['CD']:.5f}"])
    writer.writerow(["L/D Ratio", f"{result['ld_ratio']:.2f}"])
    writer.writerow(["Lift Force", f"{result['lift']:.2f} N"])
    writer.writerow(["Drag Force", f"{result['drag']:.2f} N"])
    writer.writerow(["Status", "STALL" if result["stall_warning"] else "Normal"])
    writer.writerow([])
    writer.writerow(["Formulas Used"])
    writer.writerow(["Wing Area", "S = chord x span"])
    writer.writerow(["Dynamic Pressure", "q = 0.5 x rho x V^2"])
    writer.writerow(["Lift Coefficient", "CL = cl_alpha x (alpha - alpha_0), cl_alpha = 0.1 per deg"])
    writer.writerow(["Drag Coefficient", "CD = CD_0 + k x CL^2 + thickness_drag + extra_drag"])
    writer.writerow(["Lift Force", "L = 0.5 x rho x V^2 x S x CL"])
    writer.writerow(["Drag Force", "D = 0.5 x rho x V^2 x S x CD"])
    writer.writerow(["L/D Ratio", "L/D = CL / CD"])
...
def plot_cd_vs_aoa(curve_data, current_aoa):
    ...
    plt.tight_layout()
    return fig
def plot_lift_vs_aoa(curve_data, current_aoa):
    """Plot Lift Force vs Angle of Attack."""
    fig, ax = plt.subplots(figsize=(8, 5))
    aoa = [d["aoa"] for d in curve_data]
    lift = [d["lift"] for d in curve_data]
    ax.plot(aoa, lift, 'b-', lw=2, label='Lift')
    ax.axvline(x=current_aoa, color='#f59e0b', linestyle='--', lw=1.5, label=f'Current (α={current_aoa}°)')
    ax.set_xlabel('Angle of Attack (°)', fontsize=10)
    ax.set_ylabel('Lift Force (N)', fontsize=10)
    ax.set_title('Lift vs Angle of Attack', fontsize=12, fontweight='bold')
    ax.legend(loc='best', fontsize=9)
    ax.grid(True, alpha=0.3)
    ax.set_facecolor('#fafafa')
    plt.tight_layout()
    return fig
def plot_drag_vs_aoa(curve_data, current_aoa):
    """Plot Drag Force vs Angle of Attack."""
    fig, ax = plt.subplots(figsize=(8, 5))
    aoa = [d["aoa"] for d in curve_data]
    drag = [d["drag"] for d in curve_data]
    ax.plot(aoa, drag, color='#f97316', lw=2, label='Drag')
    ax.axvline(x=current_aoa, color='#f59e0b', linestyle='--', lw=1.5, label=f'Current (α={current_aoa}°)')
    ax.set_xlabel('Angle of Attack (°)', fontsize=10)
    ax.set_ylabel('Drag Force (N)', fontsize=10)
    ax.set_title('Drag vs Angle of Attack', fontsize=12, fontweight='bold')
    ax.legend(loc='best', fontsize=9)
    ax.grid(True, alpha=0.3)
    ax.set_facecolor('#fafafa')
    plt.tight_layout()
...
# SIDEBAR NAVIGATION
# ============================================
NAV_OPTIONS = ["🏠 Home", "🎛️ Simulator", "📐 Custom Airfoil", "📊 Compare Airfoils", 
...
if page == "🏠 Home":
    ...
    with col1:
        if st.button("🚀 Start Simulation", use_container_width=True, type="primary"):
            st.session_state.nav_page = "🎛️ Simulator"
            st.rerun()
    with col2:
        if st.button("📐 Add Custom Airfoil", use_container_width=True):
            st.session_state.nav_page = "📐 Custom Airfoil"
            st.rerun()
...
# ============================================
    ...
    # Export CSV
    csv_data = generate_csv_data(airfoil, selected_name, angle_of_attack, velocity, air_density, chord_length, wing_span, result)
    st.download_button(
    ...
    with col4:
        fig4 = plot_lift_vs_aoa(curve_data, angle_of_attack)
        st.pyplot(fig4)
        plt.close()
    
    with col5:
        fig5 = plot_drag_vs_aoa(curve_data, angle_of_attack)
        st.pyplot(fig5)
        plt.close()
    ...
    with col1:
        ...
        **Dynamic Pressure:**
        ```
        q = 0.5 × ρ × V²
        q = 0.5 × {air_density} × {velocity}²
        q = {0.5*air_density*velocity**2:.2f} Pa
        ```
        """)
        st.markdown(f"""
        **Lift Coefficient:**
        ```
        CL = cl_α × (α - α₀)
        CL = 0.1 × ({angle_of_attack} - {airfoil.get('zero_lift_angle', 0)})
        CL = {result['CL']:.4f}
        ```
        """)
    with col2:
        ...
        **Drag Coefficient:**
        ```
        CD = CD₀ + k×CL² + t_drag + extra
        CD = {airfoil.get('base_drag', 0.01):.4f} + 0.01×({result['CL']:.4f})² + {airfoil.get('max_thickness', 0.12)*0.05:.4f} + ...
        CD = {result['CD']:.5f}
        ```
        """)
        st.markdown(f"""
    ...
    st.markdown(f"""
    **L/D Ratio:** L/D = CL / CD = {result['CL']:.4f} / {result['CD']:.4f} = **{result['ld_ratio']:.2f}**
    …""")
